...
export const merchProducts = pgTable("halo_merch_products", {
  id: text("id").primaryKey(),
  slug: text("slug").notNull(),
  featuredReleaseId: text("featured_release_id").notNull().default(""),
  featuredReleaseTitle: text("featured_release_title").notNull().default(""),
  artistName: text("artist_name").notNull().default("HALO"),
  collectionLabel: text("collection_label").notNull(),
  title: text("title").notNull(),
  badge: text("badge").notNull().default("HALO Merch"),
  description: text("description").notNull().default(""),
  heroImageUrl: text("hero_image_url").notNull().default(""),
  priceMinor: integer("price_minor").notNull().default(0),
  currency: text("currency").notNull().default("GBP"),
  providerName: text("provider_name").notNull().default("printful"),
  providerProductId: text("provider_product_id").notNull().default(""),
  affiliateCheckoutUrl: text("affiliate_checkout_url").notNull().default(""),
  affiliateDisclosureRequired: boolean("affiliate_disclosure_required").notNull().default(true),
  fulfillmentRegions: jsonb("fulfillment_regions").notNull().default([]),
  fulfillmentNotes: text("fulfillment_notes").notNull().default(""),
  disclosureCopy: text("disclosure_copy").notNull().default(""),
  status: text("status").notNull().default("active"),
  sortOrder: integer("sort_order").notNull().default(0),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, table => [
  uniqueIndex("halo_merch_products_slug_unique").on(table.slug),
  index("halo_merch_products_status_idx").on(table.status, table.sortOrder, table.updatedAt),
  index("halo_merch_products_release_idx").on(table.featuredReleaseId, table.updatedAt),
]);

export const merchVariants = pgTable("halo_merch_variants", {
  id: text("id").primaryKey(),
  productId: text("product_id").notNull().references(() => merchProducts.id, { onDelete: "cascade" }),
  variantLabel: text("variant_label").notNull(),
  color: text("color").notNull().default(""),
  size: text("size").notNull().default(""),
  providerSku: text("provider_sku").notNull(),
  providerVariantId: text("provider_variant_id").notNull().default(""),
  priceMinor: integer("price_minor").notNull().default(0),
  currency: text("currency").notNull().default("GBP"),
  availabilityLabel: text("availability_label").notNull().default("Available"),
  fulfillmentDaysMin: integer("fulfillment_days_min").notNull().default(2),
  fulfillmentDaysMax: integer("fulfillment_days_max").notNull().default(5),
  metadata: jsonb("metadata").notNull().default({}),
  status: text("status").notNull().default("active"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, table => [
  uniqueIndex("halo_merch_variants_product_label_unique").on(table.productId, table.variantLabel),
  uniqueIndex("halo_merch_variants_provider_sku_unique").on(table.providerSku),
  index("halo_merch_variants_status_idx").on(table.productId, table.status, table.updatedAt),
]);
