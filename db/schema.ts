...
export const songVersions = pgTable("halo_song_versions", {
  ...
  artworkUrl: text("artwork_url"),
  artworkUploadedAt: timestamp("artwork_uploaded_at", { withTimezone: true }),
  artworkBlobPrefix: text("artwork_blob_prefix").notNull().default(""),
  artworkChunkCount: integer("artwork_chunk_count").notNull().default(0),
  artworkContentType: text("artwork_content_type").notNull().default(""),
  artworkByteSize: integer("artwork_byte_size").notNull().default(0),
  artworkFilename: text("artwork_filename").notNull().default(""),
