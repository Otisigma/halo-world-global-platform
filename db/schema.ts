...
export const albumConciergeSessions = pgTable("halo_album_concierge_sessions", {
  id: uuid("id").defaultRandom().primaryKey(),
  memberId: text("member_id").notNull(),
  purpose: text("purpose").notNull().default(""),
  emotion: text("emotion").notNull().default(""),
  soundDirection: text("sound_direction").notNull().default(""),
  storyInput: text("story_input").notNull().default(""),
  generatedTitles: jsonb("generated_titles").notNull().default([]),
  selectedTitle: text("selected_title").notNull().default(""),
  generatedTheme: text("generated_theme").notNull().default(""),
  generatedWhy: text("generated_why").notNull().default(""),
  generatedStyleReferences: jsonb("generated_style_references").notNull().default([]),
  generatedTracks: jsonb("generated_tracks").notNull().default([]),
  generatedCoverPrompt: text("generated_cover_prompt").notNull().default(""),
  generatedDedication: text("generated_dedication").notNull().default(""),
  genreDirection: text("genre_direction").notNull().default(""),
  trackCount: integer("track_count").notNull().default(8),
  toneDirection: text("tone_direction").notNull().default(""),
  artworkStyle: text("artwork_style").notNull().default(""),
  finalDedication: text("final_dedication").notNull().default(""),
  coverBlobKey: text("cover_blob_key").notNull().default(""),
  coverContentType: text("cover_content_type").notNull().default(""),
  voiceNoteBlobKey: text("voice_note_blob_key").notNull().default(""),
  voiceNoteContentType: text("voice_note_content_type").notNull().default(""),
  voiceNoteFilename: text("voice_note_filename").notNull().default(""),
  shareToken: uuid("share_token").defaultRandom().notNull(),
  booklet: jsonb("booklet_json").notNull().default({}),
  unlockedExtras: jsonb("unlocked_extras").notNull().default([]),
  mode: text("mode").notNull().default("private"),
  isPremium: boolean("is_premium").notNull().default(false),
  premiumStatus: text("premium_status").notNull().default("free"),
  premiumCheckoutId: text("premium_checkout_id").notNull().default(""),
  status: text("status").notNull().default("draft"),
  errorMessage: text("error_message").notNull().default(""),
  model: text("model").notNull().default(""),
  lastRefinement: text("last_refinement").notNull().default(""),
  publishedAt: timestamp("published_at", { withTimezone: true }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, table => [
  index("halo_album_concierge_member_created_idx").on(table.memberId, table.createdAt),
  index("halo_album_concierge_status_idx").on(table.status, table.updatedAt),
  index("halo_album_concierge_mode_updated_idx").on(table.mode, table.updatedAt),
  uniqueIndex("halo_album_concierge_share_token_idx").on(table.shareToken),
]);
