...
const checks = [
  [ids(["productionHud", "productionHudTitle"]) && deck.includes("Production HUD // additive overview"), "adds a top-level production HUD section without replacing the deck"],
  ...
  [ids(["maintenanceDockPanel", "dockPanelTelemetry", "dockPanelRevision", "dockPanelTrackQr"]) && ids(["maintenanceDock", "maintenanceDockPanelTitle"]) && /id="maintenanceDock"[^>]*aria-controls="maintenanceDockPanel"/.test(deck) && /id="maintenanceDock"[^>]*aria-haspopup="dialog"/.test(deck) && /id="maintenanceDockPanel"[^>]*role="dialog"/.test(deck) && /id="maintenanceDockPanel"[^>]*aria-modal="false"/.test(deck) && /id="maintenanceDockPanel"[^>]*tabindex="-1"/.test(deck), "extends the existing floating maintenance dock with a detail panel and quick actions"],
