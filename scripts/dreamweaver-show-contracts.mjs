...
const checks = [
  ...
  [script.includes("resolveSongContextId") && script.includes("songIdFromSatellitePath") && script.includes("isSatellitePath") && script.includes("history.replaceState") && script.includes("startSatelliteAgentLoop") && script.includes("dreamweaver-satellite-"), "derives song-specific satellite context from deterministic routes, preserves satellite paths during mix hydration, and runs an isolated per-song update loop"],
