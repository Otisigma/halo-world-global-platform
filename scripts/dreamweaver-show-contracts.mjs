...
const checks = [
  ...
  [page.includes('id="songLobbyHero"') && page.includes('id="songLobbyPlayer"') && page.includes('id="showAudio"') && page.includes('id="songLobbyPlayerSource"') && page.includes('id="lobbyArtwork"') && page.includes('id="heroReelPlayer"') && page.includes("MAKE A REEL / SHORT"), "opens Dreamweaver as a hero-led Song Lobby with artwork, an embedded player, reel media, and a short-generator hook"],
  ...
  [script.includes("const AUDIO_BOOTSTRAP_TIMEOUT_MS = 15000;") && script.includes('settle({ ok: true, state: "metadata-ready" });') && script.includes('elements.audio.addEventListener("loadedmetadata", handleMetadata') && script.includes('elements.audio.addEventListener("loadeddata", handleReady') && script.includes('elements.audio.addEventListener("canplay", handleReady') && script.includes('elements.audio.addEventListener("canplaythrough", handleReady') && script.includes("elements.audio.readyState >= HTMLMediaElement.HAVE_METADATA"), "treats metadata as a slow-media bootstrap signal while preserving loaded data and canplay readiness gates"],
