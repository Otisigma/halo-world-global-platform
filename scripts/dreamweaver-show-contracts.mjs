...
const checks = [
  ...
  [page.includes('id="songLobbyHero"') && page.includes('id="songLobbyPlayer"') && page.includes('id="playBtn"') && page.includes('id="mixFileInput"') && page.includes('class="song-lobby__upload-label upload-label"') && page.includes('id="showAudio"') && page.includes('id="songLobbyPlayerSource"') && page.includes('id="lobbyArtwork"') && page.includes('id="heroReelPlayer"') && page.includes("MAKE A REEL / SHORT"), "opens Dreamweaver as a hero-led Song Lobby with artwork, upload fallback controls, reel media, and a short-generator hook"],
  ...
  [script.includes("const REMOTE_AUDIO_WATCHDOG_MS = 5000;") && script.includes('elements.mixFileInput?.addEventListener("change"') && script.includes("URL.createObjectURL(file)") && script.includes('state.audioSourceMode = "local"') && script.includes('openMixFilePicker()') && script.includes('handleRemoteAudioUnavailable("Stream unavailable — click Upload Mix File or press play to choose a local mix.")'), "guards remote playback with a five-second watchdog and promotes local uploads into the active Dreamweaver source"],
  ...
  [script.includes("state.release?.artistSlug") && script.includes("encodeURIComponent(artistSlug)") && script.includes("archiveReel"), "enriches the experience with the connected artist video archive"],
  [page.includes('id="heroReelVideo"') && script.includes("preferredHeroVideo") && script.includes('elements.heroReelPlayer') && script.includes('elements.heroReelVideo') && script.includes("showMutedHeroReel") && script.includes("HERO_REEL_LOAD_TIMEOUT_MS"), "reuses connected Dreamweaver release and video context to power a non-blocking Song Lobby hero reel with muted autoplay fallback"],
