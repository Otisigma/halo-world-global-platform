...
const redirectWildcardPrefixes = [...redirectAliases].filter(p => p.includes("*")).map(p => p.split("*")[0]);
...
async function targetExists(rawTarget, sourcePath) {
  ...
  if (cleanTarget.startsWith("/") && (redirectAliases.has(cleanTarget) || redirectWildcardPrefixes.some(prefix => cleanTarget.startsWith(prefix)))) return true;
