...
const wildcardRedirects = [...redirectAliases].filter(alias => alias.endsWith("/*")).map(alias => alias.slice(0, -1));
...
async function targetExists(rawTarget, sourcePath) {
  ...
  if (cleanTarget.startsWith("/") && (redirectAliases.has(cleanTarget) || wildcardRedirects.some(prefix => cleanTarget.startsWith(prefix)))) return true;
