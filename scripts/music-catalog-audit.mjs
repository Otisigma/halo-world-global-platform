...
const [auditLib, scout, catalogApi, cognitiveErasure, illDoItAllAgain, blessed, mySensitivityLikeACrown, mySensitivityArtworkPreservation] = await Promise.all([
  ...
  read("netlify/database/migrations/20260903000000_preserve-my-sensitivity-artwork-overrides.sql")
]);
...
assert.match(mySensitivityArtworkPreservation, /imported_artwork_url\s*=\s*COALESCE\(NULLIF\(EXCLUDED\.imported_artwork_url, ''\), halo_release_campaigns\.imported_artwork_url\)/, "follow-up migration must preserve imported_artwork_url when the seed reruns with an empty placeholder");
assert.match(mySensitivityArtworkPreservation, /artwork_override_url\s*=\s*COALESCE\(NULLIF\(EXCLUDED\.artwork_override_url, ''\), halo_release_campaigns\.artwork_override_url\)/, "follow-up migration must preserve any manual artwork override when the seed reruns");
