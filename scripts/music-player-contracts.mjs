...
assert.match(haloHome, /onLoadStart=\{\(\) => \{[\s\S]*onCanPlay=\{\(\) => \{[\s\S]*onPlaying=\{\(\) => \{[\s\S]*onError=\{\(\) => \{/, "the homepage Dreamweaver player must react to loading, ready, playing, and failure events");
