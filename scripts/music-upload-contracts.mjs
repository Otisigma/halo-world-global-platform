...
const checks = [
  ...
  [/save_version/.test(catalogClient) && /versionSongId/.test(catalogClient) && /versionId/.test(catalogClient), "keeps catalog version-save persistence used by the version editor flow"],
  [/fetch\(\s*["']\/api\/song-catalog["']\s*,/.test(catalogClient) && /fetch\(\s*["']\/api\/song-catalog\/audio["']/.test(catalogClient) && /fetch\(\s*["']\/api\/song-catalog\/artwork["']/.test(catalogClient), "music upload surface is backed by existing catalog upload APIs"],
  [/uploadHelper\.uploadChunkedFile/.test(catalogClient) && page.includes('id="audioUploadTrack"') && page.includes('id="artworkUploadTrack"') && /createUploadUi/.test(uploadHelper) && /uploadChunkedFile/.test(uploadHelper), "uses shared chunked upload behavior with visible progress states"],
