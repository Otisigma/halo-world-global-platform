...
const checks = [
  ...
  [songCatalogFn.includes('payload.action === "save_version"') && songCatalogFn.includes('path: "/api/song-catalog"') && audioFn.includes('path: "/api/song-catalog/audio"') && artworkFn.includes('path: "/api/song-catalog/artwork"'), "music upload surface is backed by the existing catalog/version/audio/artwork API routes"],
  ...
  [/\[\[headers\]\]\s+for = "\/music-upload\/\*"\s+\[headers\.values\]\s+Cache-Control = "no-cache, no-store, must-revalidate"/.test(config) && /\[\[headers\]\]\s+for = "\/upload-progress\.js"\s+\[headers\.values\]\s+Cache-Control = "no-cache, no-store, must-revalidate"/.test(config), "keeps no-cache headers for music-upload runtime freshness"],
