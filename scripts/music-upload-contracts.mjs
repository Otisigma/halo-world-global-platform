...
const checks = [
  ...
  [catalogClient.includes('action:"save_version"') && catalogClient.includes('songId:$("#versionSongId").value') && catalogClient.includes('versionId:$("#versionId").value'), "keeps catalog version-save persistence used by the version editor flow"],
  [catalogClient.includes('fetch("/api/song-catalog",options)') && catalogClient.includes('fetch("/api/song-catalog/audio"') && catalogClient.includes('fetch("/api/song-catalog/artwork"'), "music upload surface is backed by existing catalog upload APIs"],
