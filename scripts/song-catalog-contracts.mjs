...
const [page, client, styles, api, audioApi, artworkApi, producerApi, producerLib, schema, migration, audioMigration, artworkMigration, versionArtworkMigration, producerMigration, config, home, packageText, uploadHelper, satelliteHelper, pageManager] = await Promise.all([
...
const checks = [
  ...
  [api.includes("dreamweaverSatellite") && api.includes("dreamweaver-satellite.mjs") && satelliteHelper.includes("buildDreamweaverSongPage") && pageManager.includes("/dreamweaver/satellite/") && pageManager.includes("experienceUrl"), "exposes deterministic Dreamweaver satellite entry metadata in song catalog responses"],
  [Boolean(sampleDreamweaverSatellite?.route) && sampleDreamweaverSatellite.route === "/dreamweaver/satellite/11111111-1111-4111-8111-111111111111/" && sampleDreamweaverSatellite.experienceUrl.includes(sampleDreamweaverSatellite.route) && sampleDreamweaverSatellite.launchUrl === sampleDreamweaverSatellite.experienceUrl && sampleDreamweaverSatellite.pageAgent?.id === "dreamweaver-page-manager-11111111-1111-4111-8111-111111111111" && sampleDreamweaverSatellite.agentLoop?.id === sampleDreamweaverSatellite.pageAgent?.id && sampleDreamweaverSatellite.linkedSongUrl === "/music/?song=11111111-1111-4111-8111-111111111111", "shared Dreamweaver satellite helper returns deterministic route, generated page metadata, and a dedicated page manager agent for serialized API payloads"],
