  ...
  function createMonitor() {
    ...
    const render = async () => {
      ...
      activeRender = (async () => {
        ...
        launcher.dataset.state = dash.status === "green" ? "healthy" : "attention";
        launcher.textContent = `Site status: ${dash.statusLabel}${issueCount ? ` · ${issueCount} alert${issueCount === 1 ? "" : "s"}` : ""}`;
